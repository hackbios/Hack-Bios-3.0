...
export const ScrollStackItem = ({ children, itemClassName = '' }) => (
  <div className={`scroll-stack-card-wrapper ${itemClassName}`.trim()}>
...
const ScrollStack = ({
  ...
  const updateCachedOffsets = useCallback(() => {
    ...
    if (useWindowScroll) {
      cardOffsetsRef.current = cardsRef.current.map(wrapper => {
        if (!wrapper) return 0;
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
       // Using the global Lenis established in App.jsx. No isolated lenis instance needed.
